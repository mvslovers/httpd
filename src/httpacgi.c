/* httpacgi() - add path and pgm to array of HTTPROUTE in HTTPD handle
...
HTTPROUTE *httpacgi(HTTPD *httpd, const char *pgm, const char *path, int login)
{
    ...
    /* The route table lives as long as the server, and this is reachable from
       ...
       -- the HTTPROUTE, both strdup()s and the array growth -- to subpool 0
    ...
    route = calloc(1, sizeof(HTTPROUTE));
    if (!route) goto quit;

    /* set the eye catcher */
    strcpy(route->eye, HTTPROUTE_EYE);
    if (strchr(path,'*')) route->wild++;
    if (strchr(path,'?')) route->wild++;
    (void)login;                            /* retired -- see above */
    route->len  = strlen(path);
    route->path = strdup(path);
    route->pgm  = pgm ? strdup(pgm) : NULL;   /* NULL pgm == LOC route */
    ...
    if (array_add(&httpd->route, route)) {
        free(route->path);
        free(route->pgm);
        free(route);
        route = NULL;
