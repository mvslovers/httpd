...
dodd:
	...
	wtof("HTTPD417I CGI Lua script \"%s\" is not readable or does not exist", script);
	goto quit;
...
static int 
setLuaCPath(lua_State* L, const char* path)
{
	HTTPD		*httpd 	= cgihttpd();
...
static int 
process_print(HTTPD *httpd, HTTPC *httpc, const char *buf)
{
	int			rc	= 0;
	const char	*p;
...
static int 
alloc_temp(char *ddname, const char *tmpname)
{
    int         err     = 1;
    unsigned    count   = 0;
    TXT99       **txt99 = NULL;
