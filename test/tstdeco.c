...
** Contract exercised:
**   - "+"    decodes to a space
**   - "%xx"  decodes to atoe[<hex>] (the ASCII code is mapped to EBCDIC via
**            the active codepage table), consuming both hex digits
**   - ordinary bytes pass through unchanged
**   - the result is decoded in place and NUL-terminated
**
** EBCDIC note: httpdeco emits atoe[...] for a "%xx" escape, so the expected
** value of a decoded escape MUST be written as atoe[<code>] (never a bare hex
** literal) to stay correct on both cc370 (EBCDIC) and a host build. The "+",
** pass-through and length checks use character literals, which the compiler
** encodes for the target -- so they hold on either platform.
**
** The table comes from http_codepage(), which is what httpdeco() itself now
** uses (#197 -- the asc2ebc global it used to index was module storage and
** unwritable on an APF/LNKLST fetch).  With no server context to resolve,
** http_codepage() answers CP037, so this test pins CP037 explicitly rather
** than inheriting whatever a global happened to hold.
**
...
** (make test-mvs); http_codepage (HTTPCPGE) and HTTPDECO resolve from the
** [internal] autocall archive, so only the test root is listed in project.toml.
**
...
int main(void)
{
    ...
    /* a single "%xx" escape -> atoe[code], both hex digits consumed */
    strcpy((char *)buf, "%41");
    http_decode(buf);
    CHECK(strlen((char *)buf) == 1, "one escape yields one byte");
    CHECK(buf[0] == atoe[0x41], "%41 decodes to atoe[0x41]");
    ...
    CHECK(buf[0] == atoe[0x41] && buf[1] == atoe[0x42],
    ...
    CHECK(buf[0] == 'a' && buf[1] == atoe[0x20] && buf[2] == 'b',
          "%20 decodes between two literals");

    /* lowercase hex digits are accepted */
    strcpy((char *)buf, "%2f");
    http_decode(buf);
    CHECK(buf[0] == atoe[0x2f], "lowercase hex escape decodes");

    /* case-insensitivity: %2F and %2f agree */
    strcpy((char *)buf, "%2F");
    http_decode(buf);
    CHECK(buf[0] == atoe[0x2f], "uppercase hex escape decodes the same");
