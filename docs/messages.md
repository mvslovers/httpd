...
| `HTTPD048W` | `LOGIN IS RETIRED -- USE AUTH= ON EACH MOD=/LOC= ROUTE` | The member carries `LOGIN=NONE` (or a bare `LOGIN`). `NONE` was the default, so nothing changes; delete the line. |
